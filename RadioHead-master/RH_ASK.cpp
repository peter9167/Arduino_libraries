...
void RH_ASK::timerSetup()
{
 ...
 #elif defined(__arm__) && defined(__NUVOTON__)

   //Nuvoton use timer4
   void nuvoton_timer_interrupt_handler();
   Timer4.initialize((1000000/8)/_speed);
   Timer4.attachInterrupt(nuvoton_timer_interrupt_handler);
...
#elif (RH_PLATFORM == RH_PLATFORM_ARDUINO) && defined(__arm__) && defined(__NUVOTON__)
void nuvoton_timer_interrupt_handler()
{
    thisASKDriver->handleTimerInterrupt();
}
